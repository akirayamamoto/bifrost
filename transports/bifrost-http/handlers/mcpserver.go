...
func (h *MCPServerHandler) handleMCPServer(ctx *fasthttp.RequestCtx) {
	...
	// Inject MCP session identity directly into context (no headers — avoids spoofing).
...
func (h *MCPServerHandler) handleMCPServerSSE(ctx *fasthttp.RequestCtx) {
	...
	// Inject MCP session identity directly into context.
	bifrostCtx.SetValue(schemas.BifrostContextKeyIsMCPGateway, true)
	if session != nil {
		if session.AccessToken != "" {
			bifrostCtx.SetValue(schemas.BifrostContextKeyMCPUserSession, session.AccessToken)
...
func (h *MCPServerHandler) getMCPServerForRequest(ctx *fasthttp.RequestCtx) (*server.MCPServer, *tables.TablePerUserOAuthSession, error) {
	...
	if userOauthSession != nil {
		if !enforceVK && userOauthSession.VirtualKeyID == "" {
			return h.globalMCPServer, userOauthSession, nil
		}

		if userOauthSession.VirtualKeyID == "" || userOauthSession.VirtualKey == nil {
			return nil, nil, fmt.Errorf("virtual key is required in oauth session to access MCP server. Please re-authenticate with a virtual key.")
		}
	...
	if vk == "" {
		return nil, nil, fmt.Errorf("virtual key header is required to access MCP server.")
	}
...
func (h *MCPServerHandler) getPerUserOAuthSession(ctx *fasthttp.RequestCtx) (*tables.TablePerUserOAuthSession, error) {
	...
	if err != nil {
		logger.Warn("[mcp/auth] GetPerUserOAuthSessionByAccessToken error", "err", err)
		return nil, err
	}
	if session == nil {
	...
	if session.ExpiresAt.Before(time.Now()) {
		logger.Debug("[mcp/auth] Session expired", "session_id", session.ID, "expires_at", session.ExpiresAt)
		return nil, nil
	}

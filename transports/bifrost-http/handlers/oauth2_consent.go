...
func (h *ConsentHandler) RegisterRoutes(r *router.Router) {
	...
	r.GET("/api/oauth/per-user/consent/skip", h.handleSkip)
...
func (h *ConsentHandler) handleIdentityPage(ctx *fasthttp.RequestCtx) {
	...
	if !enforceVK {
		...
		<div class="option">
			...
			<a href="/api/oauth/per-user/consent/skip?flow_id=%s" class="btn btn-ghost" style="margin-top:10px">Skip</a>
		</div>`, safeFlowID)
	}
...
func (h *ConsentHandler) handleMCPsPage(ctx *fasthttp.RequestCtx) {
	...
	// All per_user_oauth MCP clients.
	perUserClients := h.store.GetPerUserOAuthMCPClients() // map[clientID -> clientName]

	safeFlowID := html.EscapeString(flowID)

	// Determine if user skipped identity selection.
	isSkipped := flow.VirtualKeyID == "" && flow.UserID == ""

	// Build MCP rows — only show connect buttons if user has an identity.
	var mcpRows strings.Builder
	if isSkipped {
		mcpRows.WriteString(`<p style="color:#6b7280;font-size:14px;">You skipped identity selection. Services will be connected when you first use their tools.</p>`)
	} else {
		for clientID, clientName := range perUserClients {
			safeID := html.EscapeString(clientID)
			...
			if !alreadyConnected && (flow.VirtualKeyID != "" || flow.UserID != "") {
				existing, _ := h.store.ConfigStore.GetOauthUserTokenByIdentity(ctx, flow.VirtualKeyID, flow.UserID, "", clientID)
...
</html>`, bifrostPageCSS, mcpRows.String(), safeFlowID))
}
...
func (h *ConsentHandler) handleSubmitVK(ctx *fasthttp.RequestCtx) {
	...
	flow.UserID = "" // Clear other identity to keep selection exclusive
...
func (h *ConsentHandler) handleSubmitUserID(ctx *fasthttp.RequestCtx) {
	...
	flow.UserID = userID
	flow.VirtualKeyID = "" // Clear other identity to keep selection exclusive
	if err := h.store.ConfigStore.UpdatePerUserOAuthPendingFlow(ctx, flow); err != nil {
...
// GET /api/oauth/per-user/consent/skip?flow_id=xxx
func (h *ConsentHandler) handleSkip(ctx *fasthttp.RequestCtx) {
	...
	if flow.VirtualKeyID != "" || flow.UserID != "" {
		flow.VirtualKeyID = ""
		flow.UserID = ""
		if err := h.store.ConfigStore.UpdatePerUserOAuthPendingFlow(ctx, flow); err != nil {
...
func (h *ConsentHandler) handleSubmit(ctx *fasthttp.RequestCtx) {
	...
	h.store.Mu.RUnlock()
	if enforceAuth && flow.VirtualKeyID == "" && flow.UserID == "" {
		redirectToIdentityPage(ctx, flowID, "An identity (Virtual Key or User ID) is required. Please choose one to continue.")
		return
	}

	// Consume the pending flow atomically: delete it now and check row count.
	// If another concurrent request already consumed it, rowsAffected will be 0.
	rowsAffected, err := h.store.ConfigStore.ConsumePerUserOAuthPendingFlow(ctx, flowID)
	if err != nil {
	...
	// 2. Migrate upstream tokens from flow proxy sessions to real session.
	if err := h.store.ConfigStore.TransferOauthUserTokensFromGatewaySession(ctx, flowID, accessToken, flow.VirtualKeyID, flow.UserID); err != nil {
		// Non-fatal: tokens can be re-acquired on first tool use.
		logger.Warn("consent submit: failed to transfer upstream tokens from flow %s: %v", flowID, err)
	}
	...
	if err := h.store.ConfigStore.CreatePerUserOAuthCode(ctx, codeRecord); err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to create authorization code: %v", err))
		return
	}

	// 4. Redirect to MCP client callback with code + original state.
	// (Pending flow was already consumed above via ConsumePerUserOAuthPendingFlow.)
...
// Satisfy the compiler — json is used via flow state serialization in future extensions.
var _ = json.Marshal

...
func (h *PerUserOAuthHandler) handleToken(ctx *fasthttp.RequestCtx) {
	...
	if codeRecord.SessionID != "" {
		...
		if err != nil {
			logger.Info("[oauth/token] Failed to load existing session", "session_id", codeRecord.SessionID, "err", err)
			sendOAuthError(ctx, fasthttp.StatusInternalServerError, "server_error", "Failed to load session")
			return
		}
		if existingSession == nil {
			logger.Info("[oauth/token] Existing session not found", "session_id", codeRecord.SessionID)

	...
	if configStore != nil {
		...
		go func() {
			...
			if err := mc.loadPricingFromDatabase(ctx); err != nil {
				syncErr = fmt.Errorf("failed to load initial pricing data: %w", err)
			}
			if err := mc.syncPricing(ctx); err != nil {
				syncErr = fmt.Errorf("failed to sync pricing data: %w", err)
			}
		}()
		go func() {
			defer wg.Done()
			if err := mc.loadModelParametersFromDatabase(ctx); err != nil {
				syncErr = fmt.Errorf("failed to load initial model parameters: %w", err)
				return
			}
			if err := mc.syncModelParameters(ctx); err != nil {
				syncErr = fmt.Errorf("failed to sync model parameters data: %w", err)
			}
		}()
		wg.Wait()
		if syncErr != nil {
...
func (mc *ModelCatalog) ReloadFromDB(ctx context.Context) error {
	...
	return mc.loadModelParametersFromDatabase(ctx)
}
...
func (mc *ModelCatalog) ForceReloadPricing(ctx context.Context) error {
	...
	go func() {
		...
		if err := mc.loadPricingOverridesFromStore(ctx); err != nil {
			syncErr = fmt.Errorf("failed to load pricing overrides: %w", err)
			return
		}
	}()
	...
	go func() {
		defer wg.Done()
		if err := mc.syncModelParameters(ctx); err != nil {
			syncErr = fmt.Errorf("failed to sync model parameters: %w", err)
			return
		}
	}()
